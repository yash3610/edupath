...
const mapStudent = (user = {}) => ({
  id: user._id || user.id,
  name: user.name || "Student",
  email: user.email || "",
  phone: user.phone || user.mobile || "-",
  avatar: user.avatar || user.profilePhoto || "",
  enrolled: Number(user.enrollments || user.enrolled || 0),
  completed: Number(user.completedCourses || user.completed || 0),
  spent: Number(user.spent || user.totalSpent || 0),
  status: user.status || (user.emailVerified ? "active" : "unverified"),
...
export default function StudentsPage() {
  ...
  const toggleStatus = async (student) => {
    ...
    try {
      const result = await apiRequest(`/api/admin/users/${student.id}/status`, {
        method: "PATCH",
        body: JSON.stringify({ status }),
      });
      const updated = mapStudent(result.data);
      setList((items) => items.map((item) => item.id === student.id ? { ...item, ...updated } : item));
  ...
  const removeStudent = async (student) => {
    try {
      await apiRequest(`/api/admin/users/${student.id}`, { method: "DELETE" });
