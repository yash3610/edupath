        ...
        <div className="mt-5 space-y-3">
          ...
          {!loading && filtered.map((quiz) => (
            <div key={quiz._id} className="grid gap-4 rounded-2xl border border-slate-200 p-4 dark:border-white/10 lg:grid-cols-[1fr_auto] lg:items-center">
              ...
              <div className="flex flex-wrap gap-2"><button onClick={() => action(quiz._id, "approve")} className="inline-flex items-center gap-2 rounded-xl bg-[#1f1c35] px-4 py-2.5 text-sm font-black text-white"><CheckCircle2 className="h-4 w-4 text-[#fec961]" /> Approve</button><button onClick={() => action(quiz._id, "reject")} className="inline-flex items-center gap-2 rounded-xl border border-[#ead9c7] bg-[#fff8ef] px-4 py-2.5 text-sm font-black text-[#9b5f18]"><XCircle className="h-4 w-4" /> Reject</button><button onClick={() => action(quiz._id, "delete", "DELETE")} className="inline-flex items-center gap-2 rounded-xl border border-[#f1d5cd] bg-[#fff4f0] px-4 py-2.5 text-sm font-black text-[#b6432d]"><Trash2 className="h-4 w-4" /> Delete</button></div>
            </div>
          ))}
