...
export default function AdminLiveClassesPage() {
  ...
  return <div className="space-y-5">
    ...
    <MotionCard className="p-4"><div className="grid gap-3 md:grid-cols-[1fr_220px]"><label className="flex items-center gap-2 rounded-xl bg-slate-50 px-4 dark:bg-white/5"><Search className="h-4 w-4 text-slate-400" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search class, course, instructor..." className="w-full bg-transparent py-3 text-sm font-bold outline-none" /></label><select value={status} onChange={(event) => setStatus(event.target.value)} className="rounded-xl border border-slate-200 px-4 dark:border-white/10 dark:bg-slate-900"><option value="">All statuses</option>{["pending-approval", "scheduled", "live", "completed", "recording-available", "cancelled", "rejected"].map((value) => <option key={value}>{value}</option>)}</select></div></MotionCard>
