  ...
  return (
    <div className="space-y-6">
      ...
      <MotionCard>
        ...
        {loading ? <p className="py-8 text-center text-sm font-bold text-slate-400">Loading quizzes...</p> : <div className="grid gap-4 lg:grid-cols-2">{quizzes.map((quiz) => <div key={quiz._id} className="rounded-2xl border border-[#eee2d6] bg-[#fffdf9] p-5 dark:border-white/10 dark:bg-white/5"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-extrabold uppercase text-[#ff723a]">{quiz.status}</p><h3 className="mt-1 text-lg font-extrabold">{quiz.title}</h3><p className="mt-1 text-sm text-slate-500">{quiz.course?.title || "Course"} - {quiz.questions?.length || 0} questions</p></div><span className="rounded-xl bg-[#fff1e8] p-2.5 text-[#ff723a]"><Icon name="BadgeHelp" /></span></div><div className="mt-5 flex flex-wrap gap-2">{quiz.status !== "published" && <button onClick={() => action(quiz, "publish")} className="rounded-lg bg-[#1f1c35] px-3 py-2 text-xs font-extrabold text-white">Publish</button>}<Link to={`/instructor/dashboard/quizzes/${quiz._id}/edit`} className="rounded-lg bg-[#fff1e8] px-3 py-2 text-xs font-extrabold text-[#c94d1e]">Edit</Link><Link to={`/instructor/dashboard/quizzes/${quiz._id}/analytics`} className="rounded-lg bg-[#f5efe8] px-3 py-2 text-xs font-extrabold text-[#514b63]">Analytics</Link><button onClick={() => action(quiz, "delete")} className="rounded-lg bg-[#fff4f0] px-3 py-2 text-xs font-extrabold text-[#b6432d]">Delete</button></div></div>)}</div>}
